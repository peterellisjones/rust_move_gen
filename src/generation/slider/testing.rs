...
pub fn test_bishop_attacks_from_bb<F: Fn(BB, BB) -> BB>(generator: F) {
    let cases = generate_test_cases_from_bb(bishop_attacks_from_sq, TEST_CASE_BOARD_DENSITY);

    for (from, occupied, expected) in cases {
        let actual = generator(from, occupied);
        assert_eq!(actual, expected);
    }
}

pub fn test_bishop_attacks_from_sq<F: Fn(Square, BB) -> BB>(generator: F) {
...
pub fn test_rook_attacks_from_bb<F: Fn(BB, BB) -> BB>(generator: F) {
...
pub fn test_rook_attacks_from_sq<F: Fn(Square, BB) -> BB>(generator: F) {
...
pub fn bench_attacks_from_sq<F: Fn(Square, BB) -> BB>(b: &mut test::Bencher, generator: F) {
    let cases = random_occupancies_from_sq(640, TEST_CASE_BOARD_DENSITY);

    b.iter(|| -> BB {
        let mut ret = EMPTY;
        for &(sq, occupied) in cases.iter() {
            ret ^= generator(sq, occupied);
        }
        ret
    });
}

pub fn bench_attacks_from_bb<F: Fn(BB, BB) -> BB>(b: &mut test::Bencher, generator: F) {
    let cases = random_occupancies_from_bb(640, TEST_CASE_BOARD_DENSITY);
    b.iter(|| -> BB {
        let mut ret = EMPTY;

        for &(from, occupied) in cases.iter() {
            ret ^= generator(from, occupied);
        }
...
fn random_occupancies_from_sq(size: usize, density: f64) -> Vec<(Square, BB)> {
    let mut ret = Vec::new();
    for i in 0..size {
        let from = Square((i % 64) as SquareInternal);
...
fn generate_test_cases_from_sq<F: Fn(Square, BB) -> BB>(
    ...
    for i in 0..64 {
        ...
        let occupied = BB::random(density) | BB::new(from);
        let expected = generator(from, occupied);
        cases.push((from, occupied, expected));
    }
    cases
}

fn generate_test_cases_from_bb<F: Fn(Square, BB) -> BB>(
    generator: F,
    density: f64,
) -> Vec<(BB, BB, BB)> {
    let mut cases = Vec::new();
    for i in 0..64 {
        ...
        let occupied = BB::random(density) | from;
        let expected = generator(from_a, occupied) | generator(from_b, occupied);
        cases.push((from, occupied, expected));
    }
    cases
}
